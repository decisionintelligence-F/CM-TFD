...
from ts_benchmark.baselines.CMTFD.utils.tools import EarlyStopping, adjust_learning_rate
...
from ts_benchmark.baselines.CMTFD.models.CMTFD_model import CMTFDModel
...
class CMTFD(ModelBase):
    def __init__(self, **kwargs):
        super(CMTFD, self).__init__()
        self.config = TransformerConfig(**kwargs)
        self.scaler = StandardScaler()
        self.seq_len = self.config.seq_len
        self.win_size = self.config.seq_len
        self.loss_weights = nn.Parameter(torch.ones(4))  # assuming 4 loss components
    ...
    def forecast_fit(self, train_valid_data: pd.DataFrame, train_ratio_in_tv: float) -> "ModelBase":
        ...
        self.model = CMTFDModel(self.config)
        ...
        #         # decoder input
        #
        #         output, loss_importance, contrast_loss = self.model(input)
        #
        #         target = target[:, -config.horizon:, :]
        #         output = output[:, -config.horizon:, :]
        #         loss = criterion(output, target)
        #
        #         total_loss = loss + loss_importance + 0.6 * contrast_loss
        #         total_loss.backward()
        ...
        def get_dynamic_alpha(epoch, strategy="warmup", max_alpha=0.1, warmup_epochs=20, delay_epochs=5):
            """
            获取当前 epoch 下的 alpha 权重，用于控制频域损失的比重。

            参数：
            - epoch: 当前训练轮数（从 0 开始）
            - strategy: "constant"（固定 alpha）、"warmup"（线性增长）、"delay"（延迟启用）
            - max_alpha: alpha 的最大值（默认 0.1）
            - warmup_epochs: warmup 总轮数（仅 warmup 模式下用）
            - delay_epochs: 延迟轮数（仅 delay 模式下用）
            ...
            if strategy == "constant":
                ...
                return min(max_alpha, epoch / warmup_epochs * max_alpha)
            elif strategy == "delay":
                return max_alpha if epoch >= delay_epochs else 0.0
            else:
                raise ValueError(f"Unknown strategy: {strategy}")

        def compute_freq_loss(output, target):
            B, T, C = output.shape
            loss = 0
            for c in range(C):
                out_c = torch.fft.rfft(output[:, :, c], dim=1)
                tgt_c = torch.fft.rfft(target[:, :, c], dim=1)
                loss += (out_c - tgt_c).abs().mean()
            return loss / C
        ...
        for epoch in range(config.num_epochs):
            ...
            for i, (input, target, input_mark, target_mark) in enumerate(train_data_loader):
                ...
                # Call the model's forward method to get the outputs
                output, loss_importance, contrast_loss, adv_loss = self.model(input)

                # Compute primary loss
                target = target[:, -config.horizon:, :]
                output = output[:, -config.horizon:, :]
                loss_time = criterion(output, target)

                # 新增频域损失
                loss_freq = compute_freq_loss(output, target)

                # 融合时频损失
                alpha = get_dynamic_alpha(epoch, strategy="warmup", max_alpha=0.05, warmup_epochs=20)
                loss = loss_time + alpha * loss_freq

                # Compute dynamic loss
                dynamic_loss = (
                    loss +
                    self.loss_weights[0] * loss_importance +
                    self.loss_weights[1] * contrast_loss +
                    self.loss_weights[2] * adv_loss
                )

                # Backpropagation
                dynamic_loss= dynamic_loss.mean()
